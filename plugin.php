...
function dd_settings_page_display() {
    ...
    echo "<p>Enter your YOURLS API signature token. This is required to authenticate the AJAX logging. You can find this token in 'Tools' page on the sidebar.</p>";
    ...
    echo "<p>Enter your <a href='https://ipinfo.io/' target='_blank'>ipinfo.io</a> API token below. This is used to map IP addresses to locations on the stats page. You can get a free token <a href='https://ipinfo.io/signup' target='_blank'>here</a>.</p>";
...
function dd_inject_collector_script( $args ) {
    ...
    $api_signature = yourls_get_option( 'dd_api_signature', '' );
    
    $endpoint = YOURLS_SITE . '/yourls-api.php?signature=' . $api_signature . '&action=log_device_info&format=json';
    ?>
...
function dd_ip_detail_page($shorturl) {
    ...
    if ($query) {
        ...
        foreach ($query as $query_result) {
            ...
            $local_time = $click_time_utc->format('h:i:s A');
            ...
            $incog = !empty($dev['in']) ? $dev['in'] : '';
            $adblk = !empty($dev['ad']) ? $dev['ad'] : '';
            ...
            $outdata .= '<tr'.$row_style.'>
                        ...
                        <td>'.$location_info.'</td>
                        <td><a href="https://who.is/whois-ip/ip-address/'.$query_result->ip_address.'" target="blank">'.$query_result->ip_address.'</a>'.$current_ip_label.'</td>
                        <td style="max-width:300px; word-wrap:break-word; word-break:break-word;">'.$ua.'</td>
        ...
        echo '<table border="1" cellpadding="5" style="margin-top:25px; border-collapse:collapse; text-align:left;">
                <tr style="background:#f1f1f1; font-weight:bold;">
                    <td>Timestamp</td><td>Local Time</td><td>Location</td>
                    <td>IP Address</td><td>User Agent</td><td>Browser/OS</td><td>Device</td><td>Engine</td><td>Referrer</td>
                    <td>Screen</td><td>Language</td><td>Battery</td><td>Incognito</td><td>AdBlock</td>
                </tr>' . $outdata . '</table><br>' . "\n\r";
